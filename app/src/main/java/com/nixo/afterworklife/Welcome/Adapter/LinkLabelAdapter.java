package com.nixo.afterworklife.Welcome.Adapter;
...
public class LinkLabelAdapter extends RecyclerView.Adapter {
    ...
    public LinkLabelAdapter(Context context, List<DataItem> list) {
    ...
    public int getItemViewType(int position) {
        if (list.get(position).getTag().contains("标题")) {
            return TITLE;
        } else {
            return CONTENT;
        }
...
//                    adapterTitleIv.setImageResource(R.drawable.circle_bule);
    ...
    private class LabelHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(final int position) {
            textView.setText(list.get(position).getTag());
            textView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (list.get(position).isSelect()) {
                        Zeng--;
                        dataList.remove(list.get(position).getId());
                        textView.setSelected(false);
                        list.get(position).setSelect(false);
                    }else{
                        ++Zeng;
                        dataList.add(list.get(position).getId());
