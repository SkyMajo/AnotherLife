...
public class Common {
     ...
     * @return
     */
    public static List<String> nearByMonthList() {

        List<String> mList = new ArrayList<>();
        mList.clear();
        for (int i = 0; i < Integer.parseInt(TimeUtils.INSTANCE.getMOUTH()); i++) {
            mList.add(months[i] + "月");
        }
        return mList;
    }
    ...
    public static List<String> dayList(String year, String month) {
        List<String> mList = new ArrayList<>();
        int day;
        Klog.Companion.e("Nixo----Debug","Year"+year+"---->"+TimeUtils.INSTANCE.getYEAR()+"----Mouth"+month+"--->"+TimeUtils.INSTANCE.getMOUTH());
        mList.clear();

        if(year.equals(TimeUtils.INSTANCE.getYEAR())&& month.equals(TimeUtils.INSTANCE.getMOUTH())){
            mList.clear();
            for (int i = 0; i < Integer.parseInt(TimeUtils.INSTANCE.getDAY()); i++) {
                mList.add(i+1+"");
            }
        }else{
        ...
        Klog.Companion.e("Nixo----Debug","result"+mList.toString());
        return mList;
    }
