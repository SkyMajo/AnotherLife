...
import com.jerey.loglib.KlogKt;
...
public class CommonIntercepter implements Interceptor {
    ...
    public synchronized static void setCommonParam(Map<String, String> commonParams) {
        if (commonParams != null) {
            if (CommonIntercepter.commonParams != null) {
                CommonIntercepter.commonParams.clear();
            } else {
                CommonIntercepter.commonParams = new HashMap<>();
            }
            for (String paramKey : commonParams.keySet()) {
                CommonIntercepter.commonParams.put(paramKey, commonParams.get(paramKey));
            }
        }
    }
    ...
    public synchronized Response intercept(Chain chain) throws IOException {
        ...
        try {
            ...
            Klog.Companion.e("Nixo","response: " + sbJson.toString());
            if(response.code() == 422)
            {
                Klog.Companion.e("NixoCode",response.code());
                HashMap<String,Object> hashMap = new HashMap<>();
                String Json = response.body().string();
                ErrorData mErrorData = new Gson().fromJson(Json, ErrorData.class);
                if(mErrorData.getErrors().getUsername().get(0) != null && !TextUtils.isEmpty(mErrorData.getErrors().getUsername().get(0))){
                    hashMap.put("msg",mErrorData.getErrors().getUsername().get(0));

                }else if(mErrorData.getErrors().getPassword().get(0) != null && !TextUtils.isEmpty(mErrorData.getErrors().getPassword().get(0))){
                    hashMap.put("msg",mErrorData.getErrors().getPassword().get(0));
                }else if(mErrorData.getErrors().getPasswordAgain().get(0) != null && !TextUtils.isEmpty(mErrorData.getErrors().getPasswordAgain().get(0))){
                    hashMap.put("msg",mErrorData.getErrors().getPasswordAgain().get(0));
                }
                hashMap.put("code","422");
                ErrorData errorData = new Gson().fromJson(sbJson.toString(), ErrorData.class);
                Klog.Companion.e("Nixo",errorData.toString());
                hashMap.put("msg",errorData.getErrors().getUsername().get(0).toString());
                response = new Response.Builder()
                        ...
                        .code(200)
                        .body(
                                ResponseBody.create(MediaType.parse("utf-8"),new Gson().toJson(hashMap))
                        ).build();
                return response;
            ...
            if(response.code() == 429){
                HashMap<String,Object> hashMap = new HashMap<>();
                hashMap.put("code","429");
                hashMap.put("msg","请求次数过多");
                response = new Response.Builder()
                        .protocol(response.protocol())
                        .message("错误拦截器转发")
                        .request(request)
            ...
            Klog.Companion.e("Nixo-->E",e.toString());
            e.printStackTrace();
            LogUtil.e(e.getMessage(), String.valueOf(e));
        }
//        saveCookies(response, request.url().toString());
//        HashMap<String,Object> hashMap = new HashMap<>();
//        hashMap.put("code","422");
//        hashMap.put("message","owowowowowowo");
        return response;
    }
