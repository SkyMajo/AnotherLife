...
class LocationGroupAdapter(var context: Context, layout:Int, list:MutableList<LocationDataItem>,var isLocal : Int) : BaseAdapter<LocationDataItem>(context,layout,list) {
    ...
    override fun initView(p0: BaseHolder, position: Int) {
        ...
        p0.itemView.click {
            ...
            intent.putExtra("userId",mDataList[position].holder.id)
        ...
        Glide.with(context).load(mDataList[position].holder.avatar?:"").into(civFqr)
        tvGroupName.text = mDataList[position].name
        tvPersonNum.text ="${mDataList[position].members}"+"/"+"${ mDataList[position].max_member}"
        tvGroupInfo.text = "${if(mDataList[position].announcement == ""){"暂无内容"}else{mDataList[position].announcement}}"
        tvFqrName.text = mDataList[position].holder.username
    }
}