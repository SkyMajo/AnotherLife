...
class GroupShenqingAdapter(var context: Context, layout:Int, list:MutableList<com.nixo.afterworklife.GroupPage.GroupManagerData.GroupApplyData.DataItem>) : BaseAdapter<com.nixo.afterworklife.GroupPage.GroupManagerData.GroupApplyData.DataItem>(context,layout,list){
    var   applyInf: applyInfs? = null
    fun setInfs(applyInfs: applyInfs):GroupShenqingAdapter{
        applyInf = applyInfs
        return this
    }
    interface applyInfs{
    ...
    override fun initView(p0: BaseHolder, position: Int) {
        ...
        p0.getView<LinearLayout>(R.id.ll_user_info).visibility = View.GONE
        p0.getView<TextView>(R.id.tv_group_name).text = mDataList[position].users.username?:""
...
//        Glide.with(context).load(mDataList[position].).into(view)
